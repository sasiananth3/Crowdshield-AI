...
def predict_with_model(model, bgr, tiled=True, tile_size=256):
    """Run tiled inference for crop-trained checkpoints and return count/map."""
    ...
    with torch.inference_mode():
        ...
        for top in range(0, h, tile_size):
            for left in range(0, w, tile_size):
                ...
                rows = max(1, min(scale, math.ceil(crop_h * scale / tile_size)))
                cols = max(1, min(scale, math.ceil(crop_w * scale / tile_size)))
...
class DensityEstimator:
    ...
    def predict(self, bgr):
        ...
        result = predict_with_model(
            self.model,
            bgr,
            tiled=self.metadata.get("inference_mode") == "tiled_256",
            tile_size=256,
        )
